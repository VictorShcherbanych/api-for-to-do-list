...
app.patch('/api/tasks/:id', async function (req, res){
    ...
    console.log(task.rows[0].name);
    let name = task.rows[0].name;
    let completedTask = name + '✅';
    const change_task = await db.query('UPDATE tasks SET name = $1 WHERE id = $2 RETURNING *', [completedTask, taskId]);
    res.send(task.rows);
